<x-app-layout>
    {{-- Removido o x-slot name="header" que não estava sendo usado --}}

    {{-- BLOCO "CRIAR POST" ADICIONADO DIRETAMENTE AQUI --}}
    @can('create', App\Models\Post::class)
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
            <div class="p-6 text-gray-900 flex items-center space-x-4">
                <a href="{{ route('profile.show', Auth::user()) }}">
                    <img class="h-10 w-10 rounded-full" src="{{ Auth::user()->image_profile ?? 'https://ui-avatars.com/api/?name='.urlencode(Auth::user()->name).'&color=7F9CF5&background=EBF4FF' }}" alt="{{ Auth::user()->name }}">
                </a>
    ...
    {{-- O resto do seu código da dashboard continua o mesmo --}}
    ...
    {{-- A barra lateral direita com as comunidades populares que você já tinha --}}
                    ...
                    @foreach($popularCommunities as $community)
                        <div class="flex items-center">
                            <a href="{{ route('communities.show', $community) }}" class="flex items-center space-x-2 group">
                                <img src="{{ $community->icon_url ?? 'https://ui-avatars.com/api/?name='.urlencode($community->name).'&color=7F9CF5&background=EBF4FF' }}" class="w-8 h-8 rounded-full">
                                <div>
